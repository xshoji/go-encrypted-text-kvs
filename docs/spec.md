...
4. encrypted KVS ファイルを一時パスへ rename する
5. macOS Keychain から該当 `key_id` の DEK を削除する
6. 一時パスの encrypted KVS ファイルを削除する

認証失敗、認証キャンセル、復号失敗、ファイル退避失敗時は何も削除しない。Keychain 削除失敗時は退避したファイルを元のパスへ戻す。成功時は stdout なしとする。
